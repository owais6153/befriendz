...
const CreateWebinars = (props) => {
  ...
  const submitHandler = async (data) => {
    ...
    if (about?.length > 0) {
      ...
      await WebinarService.addWebinar(props.token, formData)
        .then((response) => {
          navigate(`/webinars/${response.data?.data?.webinar?._id}`);
        })
        .catch((error) => {
...
function CreateWebinarPage(props) {
  return (
    <>
      <PageLayout
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(ProtectedMiddleware(CreateWebinarPage));
