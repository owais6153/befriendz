import otpServices from "../../services/web/otp.service.js";
import { createError, createResponse } from "../../utils/helper.js";

const generateOtp = async (req, res, next) => {
  try {
    const response = await otpServices.generateOtp(
      req.body.service,
      req.body.recipent
    );
...
const generateTestOtp = async (req, res, next) => {
  try {
    const response = await otpServices.generateTestOtp(
      req.body.service,
      req.body.recipent
    );
    response.message = `Otp has been sent successfully`;
...
const verifyOtp = async (req, res, next) => {
  try {
    let response = await otpServices.verifyOtp(
      req.body.otpCode,
